...
--Với mỗi đề tài, cho biết giáo viên nào tham gia đề tài đó với tổng phụ cấp lớn nhất. 
--Thông tin xuất ra gồm có tên đề tài, tên giáo viên, tổng tiền phụ cấp.
select tg.MAGV, tg.MADT, sum(tg.PHUCAP)
from GIAOVIEN gv, THAMGIADT tg
where tg.MAGV = gv.MAGV
group by MADT, tg.MAGV
having  sum(phucap) >= all(select sum(phucap)
					from GIAOVIEN gv1, THAMGIADT tg1
					where tg1.MAGV = gv1.MAGV and tg1.MADT = tg.MADT
					group by MADT, tg1.MAGV)				
